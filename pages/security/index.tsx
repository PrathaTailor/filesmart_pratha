...
export async function getStaticProps(context: GetStaticPropsContext<{ page: string[]; resolvedUrl: string }>) {
    ...
    const product = (await builder.get('co-06-product', builderSectionOptions).toPromise()) || null;
...
export default function Security({ page, product }: InferGetStaticPropsType<typeof getStaticProps>) {
    ...
    return (
        <>
            ...
            {show404 ? (
                ...
                <BuilderComponent model="co-06-product" content={product} />
            )}
        </>
    );
}
