...
export async function getStaticProps(context: GetStaticPropsContext<{ page: string[] }>) {
  const builderPageOptions = {
    userAttributes: {
      urlPath: '/' + (context.params?.page?.join('/') || 'advertorial')
    },
    options: {
      noTargeting: true
    }
  };
  const page = (await builder.get('pa-28-advertorial', builderPageOptions).toPromise()) || null;
  return {
    props: {
      page
    },
  ...
  return (
    <>
      ...
      {show404 ? (
        ...
        <BuilderComponent model="pa-28-advertorial" content={page} />
      )}
    </>
  );
}
