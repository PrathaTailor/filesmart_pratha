...
export async function getStaticProps(context: GetStaticPropsContext<{ page: string[]; resolvedUrl: string }>) {
    ...
    const service = (await builder.get('co-07-services', builderSectionOptions).toPromise()) || null;
...
export default function Services({ page, service }: InferGetStaticPropsType<typeof getStaticProps>) {
    ...
    return (
        <>
            ...
            {show404 ? (
                ...
                <BuilderComponent model="co-07-services" content={service} />
            )}
        </>
    );
}
