...
export async function getStaticProps(context: GetStaticPropsContext<{ page: string[]; resolvedUrl: string }>) {
    ...
    const benefit = (await builder.get('co-04-benefits', builderSectionOptions).toPromise()) || null;
...
export default function Benefits({ page, benefit }: InferGetStaticPropsType<typeof getStaticProps>) {
    ...
    return (
        <>
            ...
            {show404 ? (
                ...
                <BuilderComponent model="co-04-benefits" content={benefit} />
            )}
        </>
    );
}
