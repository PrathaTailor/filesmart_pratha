...
export async function getStaticProps(context: GetStaticPropsContext<{ page: string[]; resolvedUrl: string }>) {
    ...
    const footer = (await builder.get('co-17-footer', builderSectionOptions).toPromise()) || null;
...
export default function Footer({ page, footer }: InferGetStaticPropsType<typeof getStaticProps>) {
    ...
    return (
        <>
            ...
            {show404 ? (
                ...
                <BuilderComponent model="co-17-footer" content={footer} />
            )}
        </>
    );
}
