...
export async function getStaticProps(context: GetStaticPropsContext<{ page: string[] }>) {
  const builderPageOptions = {
    userAttributes: {
      urlPath: '/' + (context.params?.page?.join('/') || 'individual-tax')
    }
  };
  const page = (await builder.get('pa-29-article', builderPageOptions).toPromise()) || null;
  ...
  useEffect(() => {
    ...
    if (window.location.search) {
      ...
      async function fetchContent() {
        ...
        const content = await builder.get('pa-29-article').promise();
        setContent(content);
      }
      fetchContent();
    } else {
    ...
    <>
      ...
      {show404 ? (
        ...
        <ProductContext.Provider value={products}>
            <BuilderComponent model="pa-29-article" content={content} data={{ shortheaders: headers }} />
