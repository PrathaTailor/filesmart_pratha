...
export async function getStaticProps(context: GetStaticPropsContext<{ page: string[] }>) {
    const builderPageOptions = {
        userAttributes: {
            urlPath: '/' + (context.params?.page?.join('/') || 'contact-us')
        },
        options: {
            noTargeting: true
        }
    };
    const page = (await builder.get('pa-44-contact-us', builderPageOptions).toPromise()) || null;
    return {
        props: {
            page
        },
...
export default function Index({ page }: InferGetStaticPropsType<typeof getStaticProps>) {
    ...
    return (
        <>
            <Head>
                <title>FileSmart - Contact Us</title>
            ...
            {show404 ? (
                <DefaultErrorPage statusCode={404} />
                    ...
                    <BuilderComponent model="pa-44-contact-us" content={page} />
                </>
            )}
        </>
    );
