...
export async function getStaticProps(context: GetStaticPropsContext<{ page: string[]; resolvedUrl: string }>) {
    ...
    const headers = (await builder.get('co-02-header', builderSectionOptions).toPromise()) || null;
...
export default function Header({ page, headers }: InferGetStaticPropsType<typeof getStaticProps>) {
    ...
    return (
        <>
            ...
            {show404 ? (
                ...
                <BuilderComponent model="co-02-header" content={headers} />
            )}
        </>
    );
}
