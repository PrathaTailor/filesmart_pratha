...
export async function getStaticProps(context: GetStaticPropsContext<{ page: string[]; resolvedUrl: string }>) {
    ...
    const offerTable = (await builder.get('co-08-offer-table', builderSectionOptions).toPromise()) || null;
...
export default function OfferTable({ page, offerTable }: InferGetStaticPropsType<typeof getStaticProps>) {
    ...
    return (
        <>
            ...
            {show404 ? (
                ...
                <BuilderComponent model="co-08-offer-table" content={offerTable} />
            )}
        </>
    );
}
